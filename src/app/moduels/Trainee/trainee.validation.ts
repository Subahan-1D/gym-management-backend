...
export const createsTraineeValidationSchema = z.object({
  body: z.object({
    ...
    email: z.string().trim().nonempty({ message: 'Email is required.' }).email('Invalid email format.'),
...
export const bookClassScheduleValidationSchema = z.object({
  body: z.object({
    classScheduleId: z.string().trim().nonempty({ message: 'classScheduleId ID is required.' })
  })
});
